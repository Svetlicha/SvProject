const CACHE_NAME = 'hotel-discounts-supabase-pwa-20260824-1';
const APP_SHELL = [
  './',
  './index.html',
  './mobile.html',
  ...
  if (needsFreshCopy) {
    event.respondWith(
      fetch(event.request).then(response => {
        ...
        if (event.request.mode === 'navigate') return caches.match('./mobile.html').then(fallback => fallback || caches.match('./index.html'));
